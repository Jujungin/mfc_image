...
void IppFilterGoS(IppByteImage & imgSrc, IppFloatImage & imgDst, float sigma)
{
	...
	for (v = 1; v < h - 1; v++)
		for (z = 1; z < w - 1; z++)
		{
			h1 = -pBuf[v - 1][z - 1] - 2 * pBuf[v - 1][z] - pBuf[v - 1][z + 1]
				+ pBuf[v + 1][z - 1] + 2 * pBuf[v + 1][z] + pBuf[v + 1][z + 1];
			h2 = -pBuf[v - 1][z - 1] - 2 * pBuf[v][z - 1] - pBuf[v + 1][z - 1]
				+ pBuf[v - 1][z + 1] + 2 * pBuf[v][z + 1] + pBuf[v + 1][z + 1];

			hval = sqrt(static_cast<double>(h1 * h1 + h2 * h2));

			pDst[v][z] = static_cast<BYTE>(limit(hval + 0.5));
		}
}
